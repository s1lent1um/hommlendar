...
COMPLEX_METHODS = ['secure', 'ads', 'messages', 'likes', 'friends', 'captcha',
...
class VKError(Exception):
    ...
    def captcha_img(self):
        return self.error['captcha_img'] if 'captcha_img' in self.error else None

    @property
    def captcha_sid(self):
        return self.error['captcha_sid'] if 'captcha_sid' in self.error else None

    def __str__(self):
        str = "Error(code = '%s', description = '%s', params = '%s'" % (self.code, self.description, self.params)
        if 'captcha_sid' in self.error:
            str += ", self.captcha_sid = '%s'" % (self.captcha_sid)
        if 'captcha_img' in self.error:
            str += ", self.captcha_img = '%s'" % (self.captcha_img)
        str += ')'
        return str
